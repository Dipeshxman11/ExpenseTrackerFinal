...
async function getAllExpenses() {
  try {
    ...
    const res = await axios.get(
      "http://13.51.171.0:3001/expense/getAllExpenses/1",
      { headers: { Authorization: token } }
    );
...
async function paginationBtn(e) {
  try {
    ...
    const token = localStorage.getItem("token");
    const res = await axios.get(
      `http://13.51.171.0:3001/expense/getAllExpenses/${pageNo}`,
      { headers: { Authorization: token } }
    );
...
async function deleteExpense(e) {
  ...
  try {
    ...
    if (e.target.classList.contains("delete")) {
      ...
      const res = await axios.get(
        `http://13.51.171.0:3001/expense/deleteExpense/${id}`,
        { headers: { Authorization: token } }
      );
      window.location.reload();
    }
...
async function editExpense(e) {
  try {
    ...
    if (e.target.classList.contains("edit")) {
      ...
      const res = await axios.get(
        "http://13.51.171.0:3001/expense/getAllExpenses",
        { headers: { Authorization: token } }
      );
        ...
        if (expense.id == id) {
          ...
          addExpenseBtn.addEventListener("click", async function update(e) {
            ...
            const res = await axios.post(
              `http://13.51.171.0:3001/expense/editExpense/${id}`,
              {
...
async function buyPremium(e) {
  const token = localStorage.getItem("token");
  const res = await axios.get(
    "http://13.51.171.0:3001/purchase/premiumMembership",
    { headers: { Authorization: token } }
  );
  var options = {
    ...
    handler: async function (response) {
      const res = await axios.post(
        "http://13.51.171.0:3001/purchase/updateTransactionStatus",
        {
...
async function isPremiumUser() {
  const token = localStorage.getItem("token");
  const res = await axios.get("http://13.51.171.0:3001/user/isPremiumUser", {
    headers: { Authorization: token },
  });
